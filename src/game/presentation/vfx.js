...
import {
  BufferGeometry, BufferAttribute, Points, PointsMaterial, AdditiveBlending, NormalBlending, Color,
...
export function createImpactSparks({ scene, count = FEEDBACK.sparkPool }) {
  ...
  // The emitted colour is kept so the per-frame envelope can be RECOMPUTED from
  // it. The old loop multiplied the live colour down every frame, which is
  // destructive and made the fade shape impossible to control.
  const srcCol = new Float32Array(count * 3);
  ...
  return {
     ...
     * @param {object} [options]
     */
    burst(at, { amount = 14, speed = 2.4, spread = 0.06, color = 0xffd9a0, lifetime = 0.5, gravity = true, direction = null } = {}) {
      tint.set(color);
      ...
      for (let n = 0; n < emit; n += 1) {
        ...
        // A narrow cone along the impact vector. The old 0.8/0.32 mix threw a
        // near-hemisphere, which is what made every hit look like a party popper.
        velocities[base] = ((direction?.x ?? 0) + Math.sin(phi) * Math.cos(theta)*.16) * magnitude;
        velocities[base + 1] = Math.abs(Math.cos(phi)) * magnitude * 0.22 + 0.15;
        velocities[base + 2] = ((direction?.z ?? 0) + Math.sin(phi) * Math.sin(theta)*.16) * magnitude;
        const jitter = 0.75 + rng.next() * 0.5;
        srcCol[base] = tint.r * jitter;
        srcCol[base + 1] = tint.g * jitter;
        srcCol[base + 2] = tint.b * jitter;
    ...
    update(dt) {
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        // Attack-decay envelope. The short ramp is the droplet catching the
        // overhead key as it leaves the glove; the steep tail is it dropping out
        // of that cone. Recomputed from the emitted colour, never accumulated.
        const fade = Math.max(0, life[i] / maxLife[i]);
        const age = 1 - fade;
        const envelope = Math.min(1, age / 0.12) * fade * fade * 1.35;
        colors[base] = srcCol[base] * envelope;
        colors[base + 1] = srcCol[base + 1] * envelope;
        colors[base + 2] = srcCol[base + 2] * envelope;
      }
      alive = live;
      geometry.attributes.position.needsUpdate = true;
      geometry.attributes.color.needsUpdate = true;
