...
// Canvas-space contact ownership. The game supplies intent; Motion Forge supplies
// the swing clearance profile. Only one foot may leave contact at a time.
export function createBoxingFeet() {
  ...
  return {feet,reset,update(group,dt,speed){
    ...
    for(let i=0;i<2;i++){
      ...
      desired.set(f.sign*.225,.093,(f.sign>0?.235:-.245)*stagger).applyQuaternion(rotation).add(group.position);
    ...
    if(active<0){let largest=.20;for(let i=0;i<2;i++){const f=feet[i];desired.set(f.sign*.225,.093,(f.sign>0?.235:-.245)*(1-Math.min(1,speed))).applyQuaternion(rotation).add(group.position);const distance=f.position.distanceTo(desired);if(distance>largest){largest=distance;next=i;}}}
    ready=true;
  }};
}
