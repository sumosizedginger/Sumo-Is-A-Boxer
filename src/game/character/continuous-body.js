...
export function generateContinuousBody(landmarks,{widthScale=1,leftArmScale=1}={}){
  ...
  for(const branch of branches){
    ...
    center[1]-=.012;cap(faces,last,push(center));
...
export function skinContinuousBody(surface,character){
  ...
  for(let i=0;i<n;i++){
    const x=p[i*3],y=p[i*3+1],z=p[i*3+2],side=x>=0?'l':'r',key=x>=0?'L':'R',ax=Math.abs(x);let region,w;
    ...
    if(arm){const e=L['elbow.'+key].y,t=smooth((y-e+.055)/.11);region=y>1.40?'shoulder_'+side:Math.abs(y-e)<.055?'elbow_'+side:y>e?'upperarm_'+side:'forearm_'+side;
      const attach=smooth((y-1.35)/.17)*(1-smooth((ax-.155)/.14));
      w=[['forearm_'+side,(1-t)*(1-attach)],['upperarm_'+side,t*(1-attach)],['shoulder_'+side,attach*.6],['chest',attach*.4]];
    }else if(leg){const k=L['knee.'+key].y,t=smooth((y-k+.058)/.116),hip=smooth((y-.78)/.18);region=y>.80?'hip_'+side:Math.abs(y-k)<.065?'knee_'+side:y>k?'thigh_'+side:'calf_'+side;
      w=[['shin_'+side,(1-t)*(1-hip)],['thigh_'+side,t*(1-hip)],['pelvis',hip]];
    }else{
    ...
    regionId[i]=BODY_REGIONS[region];for(let j=0;j<4;j++){skinIndex[i*4+j]=ids[w[j]?.[0]]??0;skinWeight[i*4+j]=w[j]?.[1]??0;}
    // Continuous planar UV0 authoring chart. Overlap is explicit; no seam zero-fill.
    // Final nonoverlapping material atlas is deferred to the material turn.
  ...
  for(let pass=0;pass<24;pass++){
    ...
    for(let i=0;i<n;i++){
      ...
      for(let b=0;b<B;b++){
        const name=character.bones[b].name,wrong=(p[i*3]>0&&name.endsWith('_r'))||(p[i*3]<0&&name.endsWith('_l'));
