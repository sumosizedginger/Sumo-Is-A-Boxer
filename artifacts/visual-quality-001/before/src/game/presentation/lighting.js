...
export function createLightRig({ scene, presentation }) {
  ...
  scene.fog = new FogExp2(FOG_COLOR, 0.041);
  ...
  const hemi = new HemisphereLight(0x1d2738, 0x070809, 0.62);
  group.add(hemi);
  const ambient = new AmbientLight(0x0d131d, 0.3);
  group.add(ambient);
  ...
  return {
    ...
    update(dt) {
      ...
      for (const lamp of lamps) {
        ...
        lamp.light.intensity = lamp.base * (1 + wobble * lamp.flicker * 4 + dip + flashAmount * 0.5);
      }
    },
