...
import { PLAYER, OPPONENT, PUNCH, MATCH, RING_BOUNDS, DEFENCE } from '../config.js';
...
import { resolveStrike, conditionScaledDamage, connects } from './resolve.js';
...
export function createMatch({ playerSpawn = [0.9, 0, 2.1], opponentSpawn = [-0.9, 0, -2.1], seed = 1701 } = {}) {
  ...
  rules.addRule({
    ...
    event: 'KNOCKDOWN',
    condition: (payload) => payload.who === 'opponent' && opponent.knockdowns >= MATCH.knockdownsToLose,
    action: () => finish('WIN', 'KNOCKOUT')
  });
  rules.addRule({
    name: 'player-down-ends-match',
    event: 'KNOCKDOWN',
    condition: (payload) => payload.who === 'player' && player.knockdowns >= MATCH.knockdownsToLose,
    action: () => finish('LOSE', 'KNOCKED OUT')
  });
  rules.addRule({
    name: 'knockdown-banner',
    event: 'KNOCKDOWN',
    action: (payload) => say(payload.who === 'opponent' ? 'KNOCKDOWN!' : 'YOU ARE DOWN!', 1.6)
  });
  rules.addRule({
  ...
  function advancePlayerAction(dt) {
    ...
    if (player.action === 'down') {
      player.downT += dt;
      if (player.downT >= PLAYER.knockdownDuration) {
        player.action = 'getup';
        player.actionT = 0;
  ...
  function resolvePlayerPunch(spec) {
    ...
    if (outcome.guardBroken) {
      ...
      emit('PUNCH_LANDED', { by: 'player', punch: spec.id, outcome, at: impactPoint, heavy: spec.id === 'CROSS' });
    }

    if (opponent.health <= 0) knockdown('opponent');
  }
  ...
  function resolveOpponentStrike(strike) {
    ...
    if (outcome.guardBroken) {
      ...
      emit('PUNCH_LANDED', {
        by: 'opponent', punch: strike.attack, outcome, at: impactPoint, heavy: strike.attack === 'HOOK'
      });
    }

    if (player.health <= 0) knockdown('player');
  }
   ...
   */
  function knockdown(who) {
    if (who === 'opponent') {
      opponent.knockdowns += 1;
      opponent.health = 0;
    ...
    emit('KNOCKDOWN', { who, count: who === 'opponent' ? opponent.knockdowns : player.knockdowns });
  }
