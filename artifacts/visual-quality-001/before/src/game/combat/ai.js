...
import { OPPONENT, RING_BOUNDS } from '../config.js';
...
export function updateOpponentBrain({ opponent, player, opponentPos, playerPos, dt, rng, live }) {
  ...
  switch (opponent.state) {
    case 'down':
      if (opponent.stateT >= OPPONENT.knockdownDuration) transition(opponent, 'getup', OPPONENT.getUpDuration);
      break;
    ...
    case 'stagger':
      ...
      if (opponent.stateT >= opponent.stateDuration) transition(opponent, 'block', 0.5);
      break;
