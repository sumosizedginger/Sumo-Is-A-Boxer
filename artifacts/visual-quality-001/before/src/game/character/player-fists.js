 ...
 * the hand about the limb so the knuckles face out. Every number in POSES was
 * arrived at by looking at the running build, not by reasoning about it.
 */
...
export function createPlayerFists({ library, camera }) {
  ...
  return {
    ...
    update({ player, dt }) {
      ...
      hitShove = Math.max(0, hitShove - dt * 6);

      const pose = basePose(player);
      const swayX = Math.sin(bobPhase) * 0.013 * movement;
      const swayY = Math.sin(bobPhase * 2) * 0.017 * movement;
      const breathe = Math.sin(bobPhase * 0.43) * 0.004;

      for (const arm of [left, right]) {
        const key = arm.side === LEFT ? 'left' : 'right';
        const entry = pose[key];
        arm.targetPosition.set(entry.p[0], entry.p[1], entry.p[2]);
      ...
      for (const arm of [left, right]) {
        ...
        arm.pivot.position.set(
          arm.position.x + swayX * arm.side * -1,
          arm.position.y + swayY + breathe - hitShove * 0.06,
          arm.position.z + hitShove * 0.05
        );
        arm.pivot.rotation.set(
          arm.rotation.x + hitShove * 0.12,
          arm.rotation.y,
          arm.rotation.z + swayX * 0.6
        );
      }
    },
