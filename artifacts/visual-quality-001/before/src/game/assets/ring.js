...
function buildPlatform() {
  ...
  const deck = [
    ...
    ...repeat(4, (i, t) => slab(
      [0.16, 0.5, 0.16],
      [(t - 0.5) * 2 * (half - 0.55), RING.floorY + 0.25, -half + 0.55],
      { name: `ring-leg-n-${i}` }
    )),
    ...repeat(4, (i, t) => slab(
      [0.16, 0.5, 0.16],
  ...
  // --- canvas patchwork ---------------------------------------------------
  const tiles = 9;
  const tileSize = (half * 2) / tiles;
  const canvasTiles = [];
  const stainTiles = [];
  for (let ix = 0; ix < tiles; ix += 1) {
    for (let iz = 0; iz < tiles; iz += 1) {
      const x = -half + tileSize * (ix + 0.5);
      const z = -half + tileSize * (iz + 0.5);
      const distance = Math.max(Math.abs(x), Math.abs(z));
      // Wear is a smooth field plus a little noise, not a per-tile coin flip: a
      // coin flip produces a checkerboard, and a checkerboard is the single
      // most obvious "this was generated" tell a canvas can have. The centre of
      // a working ring is the darkest, because that is where the fight is.
      const field =
        Math.sin(x * 0.62 + 1.3) * Math.cos(z * 0.54 - 0.7) * 0.5 +
        Math.sin((x + z) * 1.05) * 0.22 +
        (1 - Math.min(1, distance / half)) * 0.75;
      const wear = field + rng.range(-0.14, 0.14);
      const tile = slab([tileSize + 0.004, 0.05, tileSize + 0.004], [x, -0.025, z], {
        name: `ring-canvas-tile-${ix}-${iz}`
      });
      if (wear > 0.78) stainTiles.push(tile); else canvasTiles.push(tile);
    }
  }
  ...
  for (const [sx, sz] of [[0, -1], [0, 1], [-1, 0], [1, 0]]) {
    ...
    for (let i = 0; i < panels; i += 1) {
      ...
      const at = along === 'x'
        ? [offset, RING.floorY + 0.5, sz * (half + 0.02)]
  ...
  RING.ropeY.forEach((y, level) => {
    ...
    for (const side of SIDES) {
      ...
      for (let i = 0; i < samples; i += 1) {
        const t = i / (samples - 1);
        const sag = Math.sin(t * Math.PI) * 0.026;
        points.push([
          ax + (bx - ax) * t,
          y - sag,
          az + (bz - az) * t
