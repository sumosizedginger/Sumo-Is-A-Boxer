...
export function createGame(host) {
  ...
  const presentationBuildMs=performance.now()-assetBuildStart-assetBuildMs;
  ...
  let hitStop = 0;
  let firstFrameMs = 0, firstFrameSinceNavigationMs = 0;
  ...
  function loop(now) {
    ...
    if (showDiagnostics) {
      ...
      hud.setDiagnostics(
        ...
        + `boot           ${firstFrameMs.toFixed(0)} ms\n`
    ...
    if(!firstFrameMs){firstFrameSinceNavigationMs=performance.now();firstFrameMs=firstFrameSinceNavigationMs-startedAt;}
  ...
  return {
    ...
    report() {
      return {
        ...
        firstFrameSinceNavigationMs:Math.round(firstFrameSinceNavigationMs),
